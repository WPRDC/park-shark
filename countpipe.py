...
def build_csv_path():
    ...
    filename = 'spaces_etl/space-counts.csv'
    csv_path = path+'/'+filename
    return csv_path

class SpacesSchema(pl.BaseSchema):
...
#    # Will this script overwrite an existing CSV file (or just append to it)?
    ...
    cumulative_resource_name = 'Space Counts and Rates'
    ...
    cumulative_spaces_pipeline = pl.Pipeline('cumulative_spaces_pipeline', 
                                          'Cumulative Spaces Pipeline', 
                                          settings_file=SETTINGS_FILE,
                                          log_status=False) \
        .connect(pl.FileConnector, csv_path) \
        .extract(pl.CSVExtractor) \
    ...
    check_and_run_pipeline(cumulative_spaces_pipeline,site,API_key,key_fields,schema,package_id,cumulative_resource_name,shoving_method)

    ##################################################################
    #if not keep_archive: # Governs whether a local archive of pulled data is kept. (Not useful for space counts.)
