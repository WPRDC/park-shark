# This script can be run to pull the last hour of parking data through the process_data 
...
def main():
    ...
    slot_start = pgh.localize(process_data.beginning_of_day(datetime(2012,7,24,0,0)))
    slot_start = pgh.localize(process_data.beginning_of_day(datetime(2013,10,1,0,0)))
...
#    halting_time = slot_start+timedelta(seconds=1)
    ...
    success = process_data.main(use_cache = False, output_to_csv = True, push_to_CKAN = False, caching_mode = 'utc_json', slot_start = slot_start, halting_time = halting_time, threshold_for_uploading = 100)
